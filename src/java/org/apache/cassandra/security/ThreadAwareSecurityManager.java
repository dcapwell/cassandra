 ...
 * (see <a href=
 * "http://cstar.datastax.com/tests/id/1d461628-12ba-11e5-918f-42010af0688f">see
 * cstar test</a>).
 * This is better than the penalty of 1 to 3 percent using a standard
 * {@code SecurityManager} with an <i>allow all</i> policy.
 * </p>
 */
public final class ThreadAwareSecurityManager extends SecurityManager {
  ...
  private static final RuntimePermission CHECK_MEMBER_ACCESS_PERMISSION = new RuntimePermission(
  ...
  public static void install() {
    ...
    // this line is needed - we need to make sure AccessControlException is loaded
    // before we install this SM
    // otherwise we may get into stackoverflow when javax.security is not allowed
    // package, and ACE is tried to be
    // loaded when it is going to be thrown from SM (class loader triggers SM to
    // verify javax.security,
  ...
  static {
    //
    // Use own security policy to be easier (and faster) since the C* has no fine
    // grained permissions.
    // Either code has access to everything or code has access to nothing (UDFs).
    // This also removes the burden to maintain and configure policy files for
    // production, unit tests etc.
    //
    ...
    // A ProtectionDomain can have its origin at an oridinary code-source or
    // provided via a
    // AccessController.doPrivileded() call.
    //
    Policy.setPolicy(new Policy() {
      public PermissionCollection getPermissions(CodeSource codesource) {
        // contract of getPermissions() methods is to return a _mutable_
  ...
  public void checkAccess(Thread t) {
    // need to override since the default implementation only checks the permission
    // if the current thread's
  ...
  public void checkPermission(Permission perm) {
    if (!DatabaseDescriptor.enableUserDefinedFunctionsThreads() && !DatabaseDescriptor.allowExtraInsecureUDFs()
        && SET_SECURITY_MANAGER_PERMISSION.equals(perm))
    ...
    // code in com.datastax.driver.core.CodecUtils uses Guava stuff, which in turns
