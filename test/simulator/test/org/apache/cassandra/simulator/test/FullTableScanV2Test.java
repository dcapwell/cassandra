 ...
 */
public class FullTableScanV2Test extends SimulationTestBase
{
    private static final Logger logger = LoggerFactory.getLogger(FullTableScanV2Test.class);
...
//        testOne(SimulationRunner.parseHex("0xd728cd5a67c8c9fa"));
            ...
            return new DTestClusterSimulation(simulated, scheduler, cluster)
            {
                ...
                protected ActionList initialize()
                {
                    ...
                    return ActionList.of(clusterActions.initializeCluster(initializeAll(cluster.size())),
                                         schemaChange(1, "CREATE KEYSPACE ks WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor' : 3}"),
                                         schemaChange(1, metadata.toCqlString(false, false, false)));
                }
